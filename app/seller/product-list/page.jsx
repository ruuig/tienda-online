...
const ProductList = () => {
  ...
  const fetchSellerProduct = async () => {
    try {
      ...
      console.log('🔍 Frontend - Token obtained:', !!token)

      // Agregar timestamp para evitar caché
      const timestamp = new Date().getTime()
      const { data } = await axios.get(`/api/product/seller-list?t=${timestamp}`, {
        headers: {
      ...
      if (data.success) {
        console.log('🔍 Frontend - Products received:', data.products.length)
  ...
  const deleteProduct = async (productId) => {
    toast((t) => (
      <div className="flex flex-col gap-3">
        ...
        <p className="text-sm text-gray-600">Esta acción eliminará permanentemente este producto de la tienda.</p>
  ...
  // Recargar productos cuando cambia el usuario o cuando se navega a esta página
  useEffect(() => {
    if (user) {
      setLoading(true)
      fetchSellerProduct();
              ...
              {filteredProducts.length > 0 ? (
                filteredProducts.map((product, index) => (
                  <tr key={index} className="border-t border-gray-500/20">
                    ...
                    <td className="px-4 py-3">Q{product.offerPrice}</td>
